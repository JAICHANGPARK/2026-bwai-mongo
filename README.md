...
행사 당일에 처음 Ollama를 설치하거나 Gemma 4 모델을 다운로드하는 방식은 피하는 것이 좋습니다.

Gemma 4 모델은 작은 모델도 수 GB라서, 현장 네트워크 상태에 따라 다운로드에 시간이 오래 걸릴 수 있습니다. 행사 전까지 다음 세 가지를 마쳐 주세요.

1. Ollama 설치 완료
2. Gemma 4 모델 다운로드 완료
3. 터미널 실행 테스트와 API 테스트 1회 성공
4. 코드 편집기를 사용할 경우 Google Antigravity 또는 기존 편집기 실행 확인

## 대상

이 문서는 Ollama로 로컬 LLM 서버와 API를 준비해야 하는 참가자 중 다음 환경을 쓰는 분을 대상으로 합니다.
...
- 코드 편집기가 필요하면 [Google Antigravity 설치 가이드](./docs/09-google-antigravity.md)를 확인하세요.
...
자세한 설치 흐름은 [Google Antigravity 설치 가이드](./docs/09-google-antigravity.md)에 있습니다.
...
문제가 생기면 [최종 체크리스트와 문제 해결](./docs/07-checklist-troubleshooting.md)를 참고하세요.
...
7. [최종 체크리스트와 문제 해결](./docs/07-checklist-troubleshooting.md)
8. [공식 참고 링크](./docs/08-references.md)
9. [Google Antigravity 설치 가이드](./docs/09-google-antigravity.md)
